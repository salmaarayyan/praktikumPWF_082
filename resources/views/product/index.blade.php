						...
						<div class="flex items-center gap-2">
							...
							@can('export-product')
								<a href="{{ route('product.export') }}"
									class="inline-flex items-center gap-2 px-4 py-2 border border-emerald-300 text-emerald-700 hover:bg-emerald-50 text-sm font-medium rounded-lg transition duration-150">
									Export
								</a>
							@endcan
						</div>
								...
								@forelse ($products as $product)
									...
									<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-100">
										...
										<td class="px-6 py-4 text-gray-600 dark:text-gray-300">
											<span
											class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $product->qty > 10 ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300' }}">
											{{ $product->qty }}
											</span>
										</td>
										...
										<td class="px-6 py-4">
											<div class="flex items-center justify-center gap-2">
												...
												@can('delete', $product)
													<x-delete-product :action="route('product.delete', $product->id)" confirm-text="Delete this product?" />
												@endcan
											</div>
										</td>
									</tr>
